...
def dus_callback(distance, publish_event, dus_settings, code="DUS", verbose=True):
    global publish_data_counter, publish_data_limit
    ...
    with counter_lock:
        dus_batch.append((dus_settings['topic'], json.dumps(distance_payload), 0, True))
...
def run_ultrasonic_door_sensor(settings, threads, stop_event, code):
    if settings['simulated']:
        ...
        dus1_thread = threading.Thread(
            target=run_dus_loop,
            args=(dus, 0.5, dus_callback, stop_event, code, publish_event, settings)
        )
        dus1_thread.start()
        threads.append(dus1_thread)

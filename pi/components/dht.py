...
def publisher_task(event, dht_batch):
    ...
    while True:
        event.wait()
        with counter_lock:
            local_dht1_batch = dht_batch.copy()
            publish_data_counter = 0
            dht_batch.clear()
...
def dht_callback(humidity, temperature, publish_event, dht_settings, code = "DHT1", verbose = False):
    ...
    humidity_payload = {
        "measurement": dht_settings['topic']+'/humidity',          
    ...
    with counter_lock:
        ...
        dht_batch.append((dht_settings['topic']+'/temperature', json.dumps(temperature_payload), 0, True))
        publish_data_counter += 1
...
def run_dht(settings, threads, stop_event, code):
        if settings['simulated']:
            ...
            dht1_thread = threading.Thread(target = run_bedroom_dht_simulator, args=(2, dht_callback, stop_event, publish_event, settings))
            dht1_thread.start()
            ...
            dht1_thread = threading.Thread(target=run_dht_loop, args=(dht, 2, dht_callback, stop_event, code))
            dht1_thread.start()
            threads.append(dht1_thread)
